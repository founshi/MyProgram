...
namespace Server
{
    ...
    /// DLL层 
    /// </summary>
    public class ServiceDBCSTypeChange : IBaseService<DBCSTypeChange>
    {
        SqlSugarClient db = null;
        public ServiceDBCSTypeChange()
        {
        ...
        {
            return db.Queryable<DBCSTypeChange>().WhereIF(whereLambda != null, whereLambda);
        }
        public ISugarQueryable<DBCSTypeChange> LoadEntities(Expression<Func<DBCSTypeChange, bool>> whereLambda, Expression<Func<DBCSTypeChange, object>> orderExpression, OrderByType type = OrderByType.Asc)
        {
            ...
            return db.Queryable<DBCSTypeChange>().WhereIF(whereLambda != null, whereLambda).OrderBy(orderExpression, type);
        }
        ...
        public List<DBCSTypeChange> LoadPageList(Expression<Func<DBCSTypeChange, bool>> whereLambda, Expression<Func<DBCSTypeChange, object>> Orderexpression, OrderByType type, int pageIndex, int pageSize, ref int totalNumber)
        {
            ...
            return db.Queryable<DBCSTypeChange>().WhereIF(whereLambda != null, whereLambda).OrderBy(Orderexpression, type).ToPageList(pageIndex, pageSize, ref totalNumber);
        }
        ...
        public int DeleteEntitysBySomeColum(List<DBCSTypeChange> _list, params Expression<Func<DBCSTypeChange, bool>>[] whereExpression)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _list.Count; i++)
                {
                    ...
                    db.Deleteable<DBCSTypeChange>().Where(_tmp).ExecuteCommand();
                }
                db.Ado.CommitTran();
            }
            catch (Exception ex)
