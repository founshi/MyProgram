...
namespace Server
{
    ...
    /// DLL层 
    /// </summary>
    public class ServiceUser_MST : IBaseService<User_MST>
    {
        SqlSugarClient db = null;
        public ServiceUser_MST()
        {
        ...
        {
            return db.Queryable<User_MST>().WhereIF(whereLambda != null, whereLambda);
        }
        public ISugarQueryable<User_MST> LoadEntities(Expression<Func<User_MST, bool>> whereLambda, Expression<Func<User_MST, object>> orderExpression, OrderByType type = OrderByType.Asc)
        {
            ...
            return db.Queryable<User_MST>().WhereIF(whereLambda != null, whereLambda).OrderBy(orderExpression, type);
        }
        ...
        public List<User_MST> LoadPageList(Expression<Func<User_MST, bool>> whereLambda, Expression<Func<User_MST, object>> Orderexpression, OrderByType type, int pageIndex, int pageSize, ref int totalNumber)
        {
            ...
            return db.Queryable<User_MST>().WhereIF(whereLambda != null, whereLambda).OrderBy(Orderexpression, type).ToPageList(pageIndex, pageSize, ref totalNumber);
        }
        ...
        public int DeleteEntitysBySomeColum(List<User_MST> _list, params Expression<Func<User_MST, bool>>[] whereExpression)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _list.Count; i++)
                {
                    ...
                    db.Deleteable<User_MST>().Where(_tmp).ExecuteCommand();
                }
                db.Ado.CommitTran();
            }
            catch (Exception ex)
